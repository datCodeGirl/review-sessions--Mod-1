const dinos = [
  ...
  // 1. Given a dinosaur object and a key return the value of the corresponding key in the object. If no key is provided return the dinosaurId
   ...
   function findDinosaurValue(obj, key){
          ...
          // using (dino[key]) will take care of an invalid key or no key at all. so we use this at that point.
   OR  
   function getValueByKeyFromDino(dino, key) {
    return key ?  dino[key] : dino.dinosaurId; // the reason we use  [] in key is because we do not know the key until we pass something as the key. dot notation is best used when we know the object (key) we are looking.

   }
  
  console.log(findDinosaurValue(dinos[0],""));
  ...
  // 2. Given a dinosaur object and an mya (Millions of years ago) value, return a boolean indicating whether it was alive during that time or not
   function findDinoAlive(dino, mya){
  ...
  console.log(findDinoAlive(dino1, 155)); // true
  
  let (dino.mya[0]= dino.mya[dino.mya[0]] - 1);
  
      
    
       dino.mya[dino.mya[1]] dino.mya[dino.mya[0]];
    }
      return boolean;
   }
  ...
  function findDinosaur(dinos, key) {
       let arr = []
        for( let i = 0; i < dinos.length; i++) {

          if (obj[key]) {
             arr.push(obj[key]) ||dino.dinosaurId
        }else {
...
console.log(findDinosaur(dinos,"period"));
console.log(findDinosaur(dinos));
